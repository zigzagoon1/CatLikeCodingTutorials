...
float4 GetFractalColor() {
	#if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED)
		float4 col;
		col.rgb = lerp(_ColorA.rgb, _ColorB.rgb, frac(unity_InstanceID * _SequenceNumbers.x + _SequenceNumbers.y));
		col.a = lerp(_ColorA.a, _ColorB.a, frac(unity_InstanceID * _SequenceNumbers.z + _SequenceNumbers.w));
		return col;
	#else
		return _ColorA;
	#endif
